...
plt.title('2D Izingov model na honeycomb resetki')
plt.legend(loc='best')
plt.savefig("figure_honeycomb%d.eps"%(numeratorfigs),dpi=400)
...
for Tc in np.linspace(1.0,2.5,150):
  binder_u4 = pyalps.collectXY(data,x='T',y='Binder Cumulant',foreach=['L'])
  for d in binder_u4:
      d.x -= Tc
      d.x = d.x/Tc
  ...
  plt.savefig('figure_honeycomb%d.eps'%(numeratorfigs),dpi=300)
  numeratorfigs+=1
