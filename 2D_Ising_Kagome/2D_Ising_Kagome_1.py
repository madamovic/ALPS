...
plt.title('2D Izingov model na Kagome resetki')
plt.legend(loc='best')
plt.savefig("figure_Kagome%d.eps"%(numeratorfigs),dpi=400)
...
f = open('binderdata_2D_Ising_Kagome.txt','w')
...
fg = open('binderdata_rounded_t_2D_Ising_Kagome.txt','w')
...
fh=open('binderdata_rounded_t_2D_Ising_Kagome.txt','w')
...
s=open('binderdata_rounded_t_redosled_2D_Ising_Kagome.txt','w')
...
for Tc in np.linspace(1.0,4.5,450):
  binder_u4 = pyalps.collectXY(data,x='T',y='Binder Cumulant',foreach=['L'])
  for d in binder_u4:
      d.x -= Tc
      d.x = d.x/Tc
  ...
  plt.savefig('figure_Kagome%d.eps'%(numeratorfigs),dpi=300)
  numeratorfigs+=1
