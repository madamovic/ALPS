...
for a in np.linspace(1.0,4.0,40):
    ...
    s=open('binder_u4_SC_redosled.txt','w')
    s.write(pyalps.plot.convertToText(binder_u4))
    s.close()
    file=open('binder_u4_SC_redosled.txt')
    ...
    plt.legend(loc='best')
    plt.savefig("figure_2_SC%d.eps"%(numeratorfigs),dpi=300)
...
for two_minus_eta in np.linspace(1.0,3.0,30):
    ...
    connected_susc=connected_susc[sel]
    s=open('connectedsusc_SC_redosled.txt','w')
    s.write(pyalps.plot.convertToText(connected_susc))
    s.close()
    file=open('connectedsusc_SC_redosled.txt')
    file_data=np.loadtxt(file,usecols=(0,1))
    x=file_data[:,0]
    y=file_data[:,1]
    for i in range(0,len(llista)):
...
for beta_over_nu in np.linspace(0.3,0.7,40):
    ...
    s=open('magnetization_abs_SC_redosled.txt','w')
    s.write(pyalps.plot.convertToText(magnetization_abs))
    s.close()
    file=open('magnetization_abs_SC_redosled.txt')
    file_data=np.loadtxt(file,usecols=(0,1))
    x=file_data[:,0]
    y=file_data[:,1]
    for i in range(0,len(llista)):
